...
public class ShopController {
    ...
    public Result getShop(String shopcode) {
        ...
        if(jsonString == null){
            ...
            Type type1 = com.google.gson.internal.$Gson$Types.newParameterizedTypeWithOwner(null, ArrayList.class, Content.class);
            List<Content> sc = new Gson().fromJson(jsonString.toString(),type1);
            Hardwarecontent h = new Hardwarecontent();
            h.setShopcode(shopcode);
            Hardwarecontent hardwarecontent = hardwarecontentService.getHardwareContentByShopcode(h);
            ...
            ShopContentMerge shop = new ShopContentMerge(sc,hardwareContentTemp);
                return ResultGenerator.genSuccessResult(shop);
            }
        }
    @PostMapping(value="/updateShop",produces = "application/json")
    public Result updateShop(HttpServletRequest request,String shopName,String shopCode,String shopBrand,String shopContent,String shopVersion){
        Type type1 = com.google.gson.internal.$Gson$Types.newParameterizedTypeWithOwner(null, ArrayList.class, Content.class);
        ...
        try{
            shopService.updateByShopCode(shop);
            request.setAttribute(LoggerUtils.LOGGER_RETURN,"更新店面成功!");
            return ResultGenerator.genSuccessResult("更新店面成功！");
        }catch(Exception e){
            e.printStackTrace();
            request.setAttribute(LoggerUtils.LOGGER_RETURN,"更新失败！请检查数据是否正确!");
            return ResultGenerator.genFailResult("更新失败！请检查数据是否正确！");
        }
    }
    ...
    public Result getReportUrl(HttpServletRequest request,String shopCode){
        String url = ProjectConstant.REPORT_SERVER_URL+"/"+shopCode+".docx";
        request.setAttribute(LoggerUtils.LOGGER_RETURN,url);
